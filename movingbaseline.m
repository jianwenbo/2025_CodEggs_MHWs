%计算时一定要注意年份范围和数组大小!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%31年的滑动阈值，其热浪时间范围(ssta,meantemp_mhw,duration,meantemp_nanmhw)1982-2085 meantemp_climate范围1982-2085
clear;clc
for y=1:24
    y 
fbl = 60;
...
for year = 1982:2085
    year
    [ssta(:,:,year-1981,:)] = cal_ssta_31(sst(:,:,(year-1982)*365+1:(year-1982)*365+31*365));
end
...
%%
%计算平均温度和气候态温度
[climate] = cal_climate(trans_shape(sst));%sst lon lat 134 365

[sst_atl] = tiqu_atl(sst);
[climate_atl] = tiqu_atl(climate);
[duration_mhw_atl,meantemp_mhw_atl] = cal_meantemp(sst_atl(:,:,16:119,:),start_mhw_atl,end_mhw_atl);
[duration_mhw_atl,meantemp_climate_atl] = cal_meantemp(climate_atl,start_mhw_atl,end_mhw_atl);
[duration_nanmhw_atl,meantemp_nanmhw_atl] = cal_meantemp(sst_atl(:,:,16:119,:),start_nanmhw_atl,end_nanmhw_atl);
%去除-1.5(根据meantemp)
...
save(['result/atlantic/',num2str(y),'.mat'],'duration_mhw_atl','meantemp_mhw_atl','meantemp_climate_atl','duration_nanmhw_atl','meantemp_nanmhw_atl','lon','lat','ci_atl','-v7.3');
...
[duration_mhw_polar,meantemp_mhw_polar] = cal_meantemp(sst_polar(:,:,16:118,:),start_mhw_polar,end_mhw_polar);
[duration_mhw_polar,meantemp_climate_polar] = cal_meantemp(climate_polar,start_mhw_polar,end_mhw_polar);
[duration_nanmhw_poalr,meantemp_nanmhw_polar] = cal_meantemp(sst_polar(:,:,16:118,:),start_nanmhw_polar,end_nanmhw_polar);
...
save(['result/polar/',num2str(y),'.mat'],'duration_mhw_polar','meantemp_mhw_polar','meantemp_climate_polar','duration_nanmhw_poalr','meantemp_nanmhw_polar','lon','lat','ci_polar','-v7.3');
clear
end
